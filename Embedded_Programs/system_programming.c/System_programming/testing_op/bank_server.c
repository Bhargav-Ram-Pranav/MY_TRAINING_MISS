...
int main()
{
	...
	mkdir("/home/engineer/Desktop/Bankdatabase",0700);
	...
	while(1)
	{
		...
		if(stater.status == 11)
		{
			...
			while(read(transfd,&trans,sizeof(trans)))
			{
				printf("the ind:%d\n",trans.ind);
				printf("the staus:%s\n",trans.status);
			...
			/*while(read(newsocketfd,&trans,sizeof(trans)))
			  {
			  send(newsocketfd,&trans,sizeof(trans),0);
			  }*/
		}
...
int directory_check(char *str)
{
	...
	struct dirent *dir;
	d=opendir("/home/engineer/Desktop/Bankdatabase");
	if(d)
	{
