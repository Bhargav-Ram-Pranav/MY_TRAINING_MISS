...
int middle_node()
{
	...
	while(temp!=NULL)
	{
		...
		ptr=(struct node **)realloc(ptr,j*sizeof(struct node *));
		if(ptr==NULL)
		{
			printf("Failed\n");
			exit(1);
...
void highest_node_number()
{
	...
	while(temp!=NULL)
	{
		if(num1 < temp->data && temp->data < num2)
		{
			num2=temp->data;
		}
		temp=temp->link;
	}
	printf("The high number:%d\n",num2);
}
