...
{
	int res=0;
	res=alloc_chrdev_region(&device_number,0,1,"pranav");
	if(res < 0)
	printk("<1>" "failed to create the device number dynamically\n");
	pr_info("Major=%d----Minor=%d\n",MAJOR(device_number),MINOR(device_number));
	dev_class=class_create(THIS_MODULE,"RAM PRANAV");
	if(dev_class == NULL)
	{
		printk("<1>" "Failed to create the class\n");
	}
        device_create(dev_class,NULL,device_number,NULL,"ram_pranav");	
