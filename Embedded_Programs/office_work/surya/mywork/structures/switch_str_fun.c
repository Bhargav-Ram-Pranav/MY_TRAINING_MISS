...
struct books_fun
{
	struct books *(*f_memory_alloc)();
	void(*f_read_inputs)(struct books *);
	void(*f_display_outputs)(struct books *);
	void(*f_swap_inputs)(struct books *);
	void(*f_dealloc)(struct books *);
};
...
int main()
{
	int prog;
	struct books *book;
	struct books_fun fun;
	fun.f_memory_alloc=memory_allocate;
	fun.f_read_inputs=read_input;
	fun.f_display_outputs=display;
	fun.f_swap_inputs=swap;
	fun.f_dealloc=deallocate;
	while(1)
        {
